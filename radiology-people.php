...
add_action( 'init', 'person', 0 );
  ...
  $people = get_posts( array(
	...
	foreach ($people as $key => $person) {
			...
			$people[$key]->overnight_attending = get_field('overnight_attending', $person->ID);			
			$people[$key]->separated = get_field('separated', $person->ID);			
			...
			$people[$key]->faceboard_image = $faceboard;
